...
namespace EditorExtensions
{
    ...
    {
        //直接挂在游戏物体上，进行逻辑代码编写以 ".cs"结尾 使用partial关键字 使一个脚本分为俩个脚本 使用,分别承担不同的作用，用以避免逻辑代码被覆盖
        public static void CreateScripts(string spawnScriptsPath, string className,bool isInterface=false,bool isEnum=false)
        {
            //创建脚本文件，并写入代码内容
            using (StreamWriter sw = File.CreateText(spawnScriptsPath))
            {
                ...
                sw.WriteLine("using UnityEngine;");
                sw.WriteLine("using UnityEngine.UI;");
                sw.WriteLine("using FrameworkDesign;");
                sw.WriteLine();
                if (!string.IsNullOrWhiteSpace(NamespaceSettings.Namespace))
                {
                    ...
                    {
                        sw.WriteLine("\tpublic enum {0} ", className);//或者 $"public class {className}: MonoBehaviour"
                        sw.WriteLine("\t{");
                        sw.WriteLine();
                        sw.WriteLine("\t}");
                    }
                    else
                    {
                        ...
                        sw.WriteLine("\t\t#region Private Field ");
                        sw.WriteLine();
                        sw.WriteLine("\t\t#endregion ");
                        sw.WriteLine();

                        sw.WriteLine("\t\t#region Public  Field ");
                        sw.WriteLine();
                        sw.WriteLine("\t\t#endregion ");
                        sw.WriteLine();

                        sw.WriteLine("\t\t#region Public Property ");
                        sw.WriteLine();
                        sw.WriteLine("\t\t#endregion ");
                        sw.WriteLine();
                        ...
                        sw.WriteLine("\t\t#region Private Method");
                        sw.WriteLine();
                        sw.WriteLine("\t\t#endregion ");
                        sw.WriteLine();

                        sw.WriteLine("\t\t#region Public Method");
                        sw.WriteLine();
                        sw.WriteLine("\t\t#endregion ");
